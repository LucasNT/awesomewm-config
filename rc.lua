...
-- battery widget
battery_widget = awful.widget.watch("cat /sys/class/power_supply/BAT0/uevent", 10 , function (widget, stdout)
    local data = {}
...
mytextclock = wibox.widget.textclock("%d/%m/%Y - %H:%M:%S")

-- Create a wibox for each screen and add it
-- Buttons:
-- 	1 left click
-- 	2 don't know , maybe mid
-- 	3 right click
-- 	4 wheel up
-- 	5 wheel down
...
awful.key({ modkey, "Control" }, "s", awesome.restart,
...
awful.key({}, "XF86AudioRaiseVolume", function ()
    awful.spawn.easy_async( "wpctl set-volume @DEFAULT_SINK@ 5%+", function ( stdout, stderr, exitreason, exitcode )
        if (exitcode == 0 ) then
            awful.spawn.easy_async( "wpctl get-volume @DEFAULT_SINK@" , update_volume )
        end
    end)
...
awful.key({modkey}, "m", function ()
    awful.spawn.easy_async( "light -A 5", function ( stdout, stderr, exitreason, exitcode )
        -- if (exitcode == 0 ) then
        -- end
    end)
end, { description = "raise brightness", group = "system"}),
awful.key({modkey}, "n", function ()
    awful.spawn.easy_async( "light -U 5", function ( stdout, stderr, exitreason, exitcode )
        -- if (exitcode == 0 ) then
        -- end
    end)
...
globalkeys = gears.table.join(
globalkeys, create_keys_for_tags('1',1),
